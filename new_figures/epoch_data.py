...
Every series is keyed by epoch and holds, per signal, AUSE and ENCE, plus the
mean AU and EU magnitudes. Epochs are sampled every 5.
"""
...
from common import (
    ...
    build_binned_rows, compute_ause, compute_ence_summary,
)
...
def _from_npz(experiment, split, offset=0):
    ...
    for i, epoch in enumerate(data["epochs"]):
        ...
        entry = {"mag_AU": float(au.mean()), "mag_EU": float(eu.mean())}
