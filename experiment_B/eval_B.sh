...
CUDA_VISIBLE_DEVICES=7 python utils/reliability.py \
  --checkpoints-dir experiment_B/checkpoints \
  --results-dir experiment_B/results \
  --experiment-name mace \
  --validation-split dataset/ani1x_system_split/cc_val.xyz \
  --test-split dataset/ani1x_system_split/cc_test_id.xyz \
  --energy-key-val "ccsd(t)_cbs.energy" \
  ...
  --output-csv-raw experiment_B/evaluation/reliability_id_cal_raw.csv \
  --output-csv-bins experiment_B/evaluation/reliability_id_cal_bins.csv \
  --output-plot experiment_B/evaluation/reliability_id_cal.png \
  --log-level INFO \
  --log-every-batches 1 \
  --device cuda \
  --batch-size 256
...
CUDA_VISIBLE_DEVICES=7 python utils/reliability.py \
  ...
  --output-csv-raw experiment_B/evaluation/reliability_id_nocal_raw.csv \
  --output-csv-bins experiment_B/evaluation/reliability_id_nocal_bins.csv \
  --output-plot experiment_B/evaluation/reliability_id_nocal.png \
  --log-level INFO \
  --log-every-batches 1 \
  --device cuda \
  --batch-size 256
...
CUDA_VISIBLE_DEVICES=7 python utils/reliability.py \
  ...
  --experiment-name mace \
  --validation-split dataset/ani1x_system_split/cc_val.xyz \
  --test-split dataset/ani1x_system_split/cc_test_ood.xyz \
  --energy-key-val "ccsd(t)_cbs.energy" \
  --energy-key-test "ccsd(t)_cbs.energy" \
  --selection-key loss \
  --selection-mode min \
  ...
  --trim 0.005 \
  --isotonic-calibration True \
  --output-csv-raw experiment_B/evaluation/reliability_ood_cal_raw.csv \
  --output-csv-bins experiment_B/evaluation/reliability_ood_cal_bins.csv \
  --output-plot experiment_B/evaluation/reliability_ood_cal.png \
  --log-level INFO \
  --log-every-batches 1 \
  --device cuda \
  --batch-size 256
...
CUDA_VISIBLE_DEVICES=7 python utils/reliability.py \
  ...
  --trim 0.005 \
  --isotonic-calibration False \
  --output-csv-raw experiment_B/evaluation/reliability_ood_nocal_raw.csv \
  --output-csv-bins experiment_B/evaluation/reliability_ood_nocal_bins.csv \
  --output-plot experiment_B/evaluation/reliability_ood_nocal.png \
  --log-level INFO \
  --log-every-batches 1 \
  --device cuda \
  --batch-size 256
...
CUDA_VISIBLE_DEVICES=7 python utils/epoch_raw.py \
  --checkpoints-dir experiment_B/checkpoints \
  --experiment-name mace \
  --split-path dataset/ani1x_system_split/cc_train.xyz \
  --energy-key "ccsd(t)_cbs.energy" \
  --device cuda \
  --batch-size 256 \
  --every-n-epochs 10 \
  --output-csv experiment_B/evaluation/epoch_raw_train.csv \
  --output-plot experiment_B/evaluation/epoch_raw_train.png
...
CUDA_VISIBLE_DEVICES=7 python utils/epoch_quality.py \
  --checkpoints-dir experiment_B/checkpoints \
  --experiment-name mace \
  --split-path dataset/ani1x_system_split/cc_test_id.xyz \
  --energy-key "ccsd(t)_cbs.energy" \
  --device cuda \
  --batch-size 256 \
  --every-n-epochs 5 \
...
CUDA_VISIBLE_DEVICES=7 python utils/epoch_quality.py \
  --checkpoints-dir experiment_B/checkpoints \
  --experiment-name mace \
  --split-path dataset/ani1x_system_split/cc_test_ood.xyz \
  --energy-key "ccsd(t)_cbs.energy" \
  --device cuda \
  --batch-size 256 \
  --every-n-epochs 5 \
...
CUDA_VISIBLE_DEVICES=7 python utils/epoch_quality_finetune_same_dataset.py \
  --pretrain-checkpoints-dir experiment_A/checkpoints \
  --pretrain-experiment-name mace \
  --finetune-checkpoints-dir experiment_B/checkpoints \
...
CUDA_VISIBLE_DEVICES=7 python utils/epoch_quality_finetune_mixed_dataset.py \
  --pretrain-checkpoints-dir experiment_A/checkpoints \
  --pretrain-experiment-name mace \
  --pretrain-split-path dataset/ani1x_system_split/dft_test_id.xyz \
  --pretrain-energy-key wb97x_tz.energy \
  --finetune-checkpoints-dir experiment_B/checkpoints \
  --finetune-experiment-name mace \
  --finetune-split-path dataset/ani1x_system_split/cc_test_id.xyz \
...
CUDA_VISIBLE_DEVICES=7 python utils/epoch_quality_finetune_mixed_dataset.py \
  --pretrain-checkpoints-dir experiment_A/checkpoints \
  --pretrain-experiment-name mace \
  --pretrain-split-path dataset/ani1x_system_split/dft_test_ood.xyz \
  --pretrain-energy-key wb97x_tz.energy \
  --finetune-checkpoints-dir experiment_B/checkpoints \
  --finetune-experiment-name mace \
  --finetune-split-path dataset/ani1x_system_split/cc_test_ood.xyz \
  --finetune-energy-key "ccsd(t)_cbs.energy" \
  --device cuda \
  --batch-size 256 \
  --every-n-epochs-pretrain 5 \
...
CUDA_VISIBLE_DEVICES=7 python utils/epoch_quality_finetune_mixed_dataset.py \
  --pretrain-checkpoints-dir experiment_A/checkpoints \
  --pretrain-experiment-name mace \
  --pretrain-split-path dataset/ani1x_system_split/dft_train.xyz \
  --pretrain-energy-key wb97x_tz.energy \
  --finetune-checkpoints-dir experiment_B/checkpoints \
  --finetune-experiment-name mace \
  --finetune-split-path dataset/ani1x_system_split/cc_train.xyz \
  --finetune-energy-key "ccsd(t)_cbs.energy" \
  --device cuda \
  --batch-size 256 \
  --every-n-epochs-pretrain 5 \
...
CUDA_VISIBLE_DEVICES=7 python utils/finetune.py \
  --pretrain-checkpoints-dir experiment_A/checkpoints \
  --pretrain-results-dir experiment_A/results \
  --pretrain-experiment-name mace \
  --finetune-checkpoints-dir experiment_B/checkpoints \
  --finetune-results-dir experiment_B/results \
  --finetune-experiment-name mace \
  --test-id-split dataset/ani1x_system_split/cc_test_id.xyz \
  --test-ood-split dataset/ani1x_system_split/cc_test_ood.xyz \
  --energy-key-test "ccsd(t)_cbs.energy" \
  ...
  --device cuda \
  --batch-size 256 \
  --output-csv experiment_B/evaluation/finetune.csv \
  --output-plot experiment_B/evaluation/finetune.png
...
CUDA_VISIBLE_DEVICES=7 python utils/distribution.py \
  ...
  --device cuda \
  --batch-size 256 \
  --output-csv experiment_B/evaluation/distribution_id.csv \
  --output-plot experiment_B/evaluation/distribution_id.png
...
CUDA_VISIBLE_DEVICES=7 python utils/distribution.py \
  ...
  --device cuda \
  --batch-size 256 \
  --output-csv experiment_B/evaluation/distribution_ood.csv \
  --output-plot experiment_B/evaluation/distribution_ood.png
...
CUDA_VISIBLE_DEVICES=7 python utils/distribution.py \
  ...
  --device cuda \
  --batch-size 256 \
  --output-csv experiment_B/evaluation/distribution_train.csv \
  --output-plot experiment_B/evaluation/distribution_train.png
